...
class OursTrainer(object):
    ...
    def update_exp_avg_loss(self, group_loss, group_count):
        prev_weights = (1 - self.gamma * (group_count > 0).float()).to(self.device) * (
                    self.exp_avg_initialized > 0).float().to(self.device)
    ...
    def train(self, inputs, targets, y_Assistance=None):
        ...
        if glo.get_value('model_name') == 'MyRKan':
            ...
            # loss = self.loss(output.to(self.device).squeeze(), targets.to(self.device).squeeze()).mean()

            actual_loss = self.get_loss(output, targets, data_flag)  # * 0.5 + loss*0.5
            # loss = self.loss(y_model,targets)
            # loss = self.loss(output.to(self.device).squeeze(), targets.to(self.device).squeeze()).mean()
            # Var_P = {'0.0': [], '3.0': [], '7.0': []}
            # loss2  = 0;
            # for i in range(len(targets)):
            #     mask = (targets == targets[i]).squeeze() & (y_Assistance != y_Assistance[i]).detach().cpu()
            #     if len(torch.nonzero(mask).squeeze()) > 2:
            #         matching_indices = torch.nonzero(mask).squeeze()
            #         shuffled_indices = matching_indices # matching_indices[torch.randperm(matching_indices.size(0))]
            #         matching_indices1 = shuffled_indices[:2] # 取前5个
            #     else:
            #         matching_indices1 = mask
            #     # 提取输出值
            #     positive_sample_i = output[matching_indices1]
            #
            #     # mask = (targets != targets[i]).squeeze() & (y_Assistance == y_Assistance[i]).detach().cpu()
            #     # if len(torch.nonzero(mask).squeeze()) > 2:
            #     #     matching_indices = torch.nonzero(mask).squeeze()
            #     #     shuffled_indices = matching_indices[torch.randperm(matching_indices.size(0))]
            #     #     matching_indices = shuffled_indices[:2] # 取前5个
            #     # else:
            #     #     matching_indices = mask
            #     # 提取输出值
            #     # negtive_sample_i = output[matching_indices]
            #
            #     loss2 += self.triplet_loss(output[i],positive_sample_i)/len(targets)
            # loss = loss + loss1

            #     Var_P[f'{torch.round(data_flag[i] * 10)}'].append(positive_sample_i) # tensor([1.0536, 0.1567, 1.7525], grad_fn=<SelectBackward0>)
            # loss = self.loss(output.to(self.device).squeeze(), targets.to(self.device).squeeze())
            # Lkan_reg = sum([self.reg(self.model.LKan[i] ) for i in range(4)])
            # Lkan_reg = self.reg(self.model.LKan)
            # Resultkan_reg = self.reg(self.model.ResultKan)
            # Rkan_reg = self.reg(self.model.RKan)
            # reg = (Rkan_reg + Lkan_reg + Resultkan_reg) * 0

            # variance = 0;
            # # Group DRO loss
            # for key, tensor_list in Var_P.items():
            #     if len(tensor_list) <= 1 :
            #         continue
            #     # 将张量列表堆叠为一个张量
            #     stacked_tensors = torch.stack(tensor_list)  # 堆叠后形状为 [n, 1]
            #     # 计算方差并除以元素数量
            #     if stacked_tensors.numel() <= 1:
            #         continue
            #     variance = torch.sqrt(1000*stacked_tensors.var() / stacked_tensors.numel())
            #     if torch.isnan(variance):
            #         continue
            #     loss += variance

            # (actual_loss+loss2*0+ reg).backward()
            #
            ...
            # 分组计算方差
            # unique_flags = torch.unique(data_flag)  # 获取 flag 的唯一值
            # variance_dict = {}
            # loss_flag = torch.nn.functional.mse_loss(output, targets, reduction='none')
            # for flag in unique_flags:
            #     group_loss = loss_flag[data_flag == flag]  # 筛选出对应 flag 的 loss
            #     variance = torch.sqrt(10*torch.var(group_loss, unbiased=False))  # 计算方差（无偏估计为 False）
            #     loss += float(variance)  # 保存结果

            (loss).backward()
        elif glo.get_value('model_name') == 'TimeKAN':
    ...
    def get_loss(self, output, targets, data_flag):
        ...
        if self.is_robust and not self.mix:
            ...
            actual_loss, weights = self.compute_robust_loss(group_loss, group_count)
            actual_loss = 0.5 * group_loss.mean() + 0.5 * actual_loss
        else:
    ...
    def _run(self, inputs):
        ...
        if glo.get_value('model_name') == 'MyRKan':
            ...
            inputs = inputs.transpose(0, 1).to(self.device)
            outputs = torch.nn.functional.sigmoid(self.model(inputs[-1]))
            inputs = inputs.transpose(0, 1)
...
def train_model(
    ...
    datasets['train'].inputs[:,:,2:] = (datasets['train'].inputs[:,:,2:] - np.array(min[2:])) / (np.array(max[2:]) - np.array(min[2:]) + 1)

    datasets['val'].inputs[:,:,2:] = (datasets['val'].inputs[:,:,2:] - np.array(min[2:])) / (np.array(max[2:]) - np.array(min[2:]) + 1)

    datasets['test'].inputs[:,:,2:] = (datasets['test'].inputs[:,:,2:] - np.array(min[2:])) / (np.array(max[2:]) - np.array(min[2:]) + 1)

    datasets['train'].inputs[:, :, :2] = datasets['train'].inputs[:, :, :2] - np.array(min[:2])

    datasets['val'].inputs[:, :, :2]= (datasets['val'].inputs[:, :, :2] - np.array(min[:2]))

    datasets['test'].inputs[:, :, :2] = datasets['test'].inputs[:, :, :2] - np.array(min[:2])

    # datasets['val']['Anki'].inputs = (datasets['val']['Anki'].inputs - np.array(min))/(np.array(max)-np.array(min)+1)
    # datasets['val']['Duolingo'].inputs = (datasets['val']['Duolingo'].inputs - np.array(min)) / ( np.array(max) - np.array(min) + 1)
    # datasets['val']['MeiMemo'].inputs = (datasets['val']['MeiMemo'].inputs - np.array(min)) / ( np.array(max) - np.array(min) + 1)

    # datasets['test']['Anki'].inputs = (datasets['test']['Anki'].inputs - np.array(min[:-1])) / (np.array(max[:-1]) - np.array(min[:-1]) + 1)
    # datasets['test']['Duolingo'].inputs = (datasets['test']['Duolingo'].inputs - np.array(min[:-1])) / (np.array(max[:-1]) - np.array(min[:-1]) + 1)
    # datasets['test']['MeiMemo'].inputs = (datasets['test']['MeiMemo'].inputs - np.array(min[:-1])) / (np.array(max[:-1]) - np.array(min[:-1]) + 1)
    ...
    ## fit globalKmean
    # if glo.get_value('model_name') == 'MyRKan':
    #     trainer.model.fit_myKMean(datasets['train'].inputs);

    Assistance_model = Assisant
    # # 切换到评估模式（如果是用于推理）
    # Assistance_model.eval()
