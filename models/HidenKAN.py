...
class HidenKan(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(), symbolic_enabled=True, bias_trainable=True, grid_eps=1.0, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True,
        ...
        for l in range(self.depth):
            ...
            # 修改处
            # sb_batch = KANLayer(in_dim=width[l], out_dim=width[l + 1], device=device)
    ...
    #             optimizer.step()
    #
    #         test_loss = loss_fn_eval(self.forward(torch.tensor(dataset['test'].inputs[test_id,:]).to(device)), torch.tensor(dataset['test'].targets[test_id]).to(device))
    #         if _ % log == 0:
    ...
    def prune(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        ...
        model2 = HidenKan(copy.deepcopy(self.width), self.grid, self.k, base_fun=self.base_fun, device=self.device)
