...
class RKAN(nn.Module):
    ...
    def forward(self, x):
        ...
        batch = x.shape[1]
        time = x.shape[0]
        self.h0 = Variable(torch.zeros(batch, 3))
        ...
        self.acts_scale_std = []
        self.neurons_scale = []
        self.acts.append(torch.cat([x[-1, :, :], self.h0],dim = -1))  # acts shape: (batch, width[l])
        ans = []
        for i in range(time):
            self.data = torch.cat([x[i, :, :], self.h0],dim = -1)
            self.h0 = self.loop_kanlayer(0,self.data,i,time)
            ans.append(self.h0);

        return ans
    def loop_kanlayer(self,x,data,i,time):
        for l in range(self.depth):
            x_numerical, preacts, postacts_numerical, postspline = self.act_fun[l](data)
            if self.symbolic_enabled == True:
                x_symbolic, postacts_symbolic = self.symbolic_fun[l](data)
            else:
                x_symbolic = 0.
                postacts_symbolic = 0.
            data = x_numerical + x_symbolic
            ...
            # my_cheby_scale = torch.sum(torch.abs(grid_reshape), axis=-1)

            self.data = data + self.biases[l].weight
            if (l == self.depth - 2):
                self.h0 = data
            if (i == time - 1):
                self.acts_scale.append(output_range / input_range)
                # self.acts_scale.append(my_cheby_scale)
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0.,
        ...
        if batch == -1 or batch > dataset['train_input'].shape[0]:
            batch_size = dataset['train'].inputs.shape[0]
            batch_size_test = dataset['test'].inputs.shape[0]
        else:
            batch_size = batch
        ...
        def closure():
            global train_loss, reg_
            optimizer.zero_grad()
            pred = self.forward(torch.tensor(torch.tensor(dataset['train'].inputs[train_id, :])).to(device))
            if sglr_avoid == True:
                id_ = torch.where(torch.isnan(torch.sum(pred, dim=1)) == False)[0]
                train_loss = loss_fn(pred[id_], dataset['train_label'][train_id][id_].to(device))
            else:
                train_loss = loss_fn(pred, torch.tensor(dataset['train'].targets[train_id]).to(device))
        ...
        for _ in pbar:
            ...
            train_id = np.random.choice(dataset['train'].inputs.shape[0], batch_size, replace=False)
            test_id = np.random.choice(dataset['test'].inputs.shape[0], batch_size_test, replace=False)

            if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid:
                self.update_grid_from_samples(torch.tensor(torch.tensor(dataset['train'].inputs[train_id, :])).to(device))
            if opt == "LBFGS":
            ...
            if opt == "Adam":
                ...
                if sglr_avoid == True:
                    id_ = torch.where(torch.isnan(torch.sum(pred, dim=1)) == False)[0]
                    train_loss = loss_fn(pred[id_], torch.tensor(dataset['train'].targets[train_id]).to(device))
                else:
                    train_loss = loss_fn(pred, torch.tensor(dataset['train'].targets[train_id]).to(device))
                reg_ = reg(self.acts_scale)
            ...
            test_loss = loss_fn_eval(self.forward(torch.tensor(dataset['test'].inputs[test_id, :]).to(device)),
                                     torch.tensor(dataset['test'].targets[test_id]).to(device))
            if _ % log == 0:
    ...
    def prune(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        ...
        model2 = RKAN(copy.deepcopy(self.width), self.grid, self.k, base_fun=self.base_fun, device=self.device)
